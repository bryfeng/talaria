...
class TestHandleMessage:
    ...
    def test_board_command(self):
        with patch.object(ui, "api", return_value={"cards": [], "columns": []}) as mock_api, \
            ...
            assert "reply_markup" in mock_send.call_args.kwargs
    ...
    def test_find_command(self):
        board = {
            "cards": [
                {"id": "ab12", "title": "Fix telegram", "description": "", "labels": ["domain:telegram"], "column": "ready"},
                {"id": "cd34", "title": "Watcher update", "description": "", "labels": [], "column": "backlog"},
            ],
            "columns": [],
        }
        with patch.object(ui, "api", return_value=board), \
             patch.object(ui, "tg_send") as mock_send:
            ui.handle_message(self._msg("/find tele"))
            mock_send.assert_called_once()
            assert "ab12" in mock_send.call_args[0][1]

    def test_move_command(self):
        card = {"id": "ab12", "title": "T", "column": "review", "priority": "medium", "labels": [], "description": ""}
        with patch.object(ui, "api", return_value=card) as mock_api, \
             patch.object(ui, "tg_send") as mock_send:
            ui.handle_message(self._msg("/move ab12 review"))
            mock_api.assert_called_once_with("PATCH", "/api/card/ab12", {"column": "review"})
            mock_send.assert_called_once()
...
class TestHandleCallback:
    ...
    def test_move_action(self):
        ...
        with patch.object(ui, "FEEDBACK_MODE", "off"), \
             patch.object(ui, "api", return_value=fake_card) as mock_api, \
    ...
    def test_board_next_action(self):
        board = {
            "cards": [{"id": "r1", "column": "ready", "priority": "high", "created_at": "2026-01-01"}],
            "columns": [],
        }
        full = {"id": "r1", "title": "Ready card", "column": "ready", "priority": "high", "labels": [], "description": ""}
        with patch.object(ui, "api", side_effect=[board, full]) as mock_api, \
             patch.object(ui, "tg_answer_callback"), \
             patch.object(ui, "tg_edit") as mock_edit:
            ui.handle_callback(self._cb("talaria:board:next"))
            assert mock_api.call_count == 2
            mock_edit.assert_called_once()

    def test_board_refresh_action(self):
        board = {"cards": [], "columns": []}
        with patch.object(ui, "api", return_value=board), \
             patch.object(ui, "tg_answer_callback"), \
             patch.object(ui, "tg_edit") as mock_edit:
            ui.handle_callback(self._cb("talaria:board:refresh"))
            mock_edit.assert_called_once()

    def test_api_error_answered(self):
        with patch.object(ui, "FEEDBACK_MODE", "off"), \
             patch.object(ui, "api", side_effect=RuntimeError("boom")), \
             patch.object(ui, "tg_answer_callback") as mock_ans:
            ui.handle_callback(self._cb("talaria:move:ee55:spec"))
            mock_ans.assert_called_once()

    def test_done_gate_error_shows_missing_requirements(self):
        err = ui.ApiError("PATCH", "/api/card/bb22", 400, {"missing": ["type label", "done summary"]})
        with patch.object(ui, "FEEDBACK_MODE", "off"), \
             patch.object(ui, "api", side_effect=err), \
             patch.object(ui, "tg_answer_callback") as mock_ans, \
             patch.object(ui, "tg_send") as mock_send:
            ui.handle_callback(self._cb("talaria:done:bb22:"))
            assert mock_ans.call_count >= 1
            mock_send.assert_called_once()
            assert "Missing requirements" in mock_send.call_args[0][1]

    def test_done_sends_feedback_in_important_mode(self):
        done_card = {"id": "bb22", "title": "T", "column": "done", "priority": "medium", "labels": [], "description": ""}
        board = {"cards": [{"id": "x", "column": "done"}, {"id": "y", "column": "done"}], "columns": []}

        with patch.object(ui, "FEEDBACK_MODE", "important"), \
             patch.object(ui, "api", side_effect=[done_card, board]) as mock_api, \
             patch.object(ui, "tg_answer_callback"), \
             patch.object(ui, "tg_edit"), \
             patch.object(ui, "tg_send") as mock_send:
            ui.handle_callback(self._cb("talaria:done:bb22:"))
            ...
            assert mock_api.call_count == 2
            mock_send.assert_called_once()
            assert "Done now: 2/20" in mock_send.call_args[0][1]

    def test_move_sends_feedback_in_all_mode(self):
        card = {"id": "aa11", "title": "T", "column": "ready", "priority": "medium", "labels": [], "description": ""}

        with patch.object(ui, "FEEDBACK_MODE", "all"), \
             patch.object(ui, "api", return_value=card), \
             patch.object(ui, "tg_answer_callback"), \
             patch.object(ui, "tg_edit"), \
             patch.object(ui, "tg_send") as mock_send:
            ui.handle_callback(self._cb("talaria:move:aa11:ready"))
            ...
            mock_send.assert_called_once()
            assert "Moved aa11 to ready" in mock_send.call_args[0][1]
