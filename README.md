...
|---------|:------:|:------:|:---------------:|:-------:|
| Agent auto-spawn from card movement | No | No | No | **Yes** |
| Isolated git worktree per card | No | No | No | **Yes** |
| Review gate with per-card checks | No | No | Partial | **Yes** |
| Markdown cards as source of truth | No | Partial | No | **Yes** |
| Runs without an external database | No | No | No | **Yes** |
...
# 3) Create and move a card
talaria create "Demo: telegram UX polish"
talaria move <card_id> in_progress

# 4) Observe progress
talaria log <card_id>
talaria history telegram --limit 5
```

### Troubleshooting

- `Talaria API not reachable`: confirm `talaria-server` is running on `http://localhost:8400`.
- `No automated movement`: ensure `agent_watcher.py` is running and the card meets column requirements.
- `Telegram command not routed`: verify `TALARIA_BASE_URL` and `TALARIA_TELEGRAM_UI_ENABLED=true` (for Hermes gateway mode).
- `Stable/dev confusion`: reinstall editable package from your intended runtime clone, then restart server/watcher.
...
- /board, /next, /card, /find, /create, /move, /note, /history are routed API-first to Talaria
...
| `TALARIA_HOME` | `~/.talaria/talaria` | Optional alternate Talaria instance directory |
| `TALARIA_WORK_DIR` | `~/talaria` | Working directory for agents/worktrees |
